...
class BFA(object):
    ...
    def flip_bit(self, n, m):
        ...
        w_grad_topk, w_idx_topk = m.weight.grad.detach().abs().view(-1).topk(
            self.k_top)
        # --------------------------
        # w_idx_top1 = w_idx_topk[0]
        # grad_top1_f2 = 0
        # grad_top1_f1 = 0
        # grad_top1_b1 = 0
        # grad_top1_b2 = 0
        # length = len(m.weight.grad.detach().view(-1))
        # if w_idx_top1-2 > 0:
        #     grad_top1_f2 = m.weight.grad.detach().view(-1)[w_idx_top1-2].float()
        # if w_idx_top1-1 > 0:
        #     grad_top1_f1 = m.weight.grad.detach().view(-1)[w_idx_top1-1].float()
        # grad_top1 = m.weight.grad.detach().view(-1)[w_idx_top1].float()
        # if w_idx_top1+1 < length:
        #     grad_top1_b1 = m.weight.grad.detach().view(-1)[w_idx_top1+1].float()
        # if w_idx_top1+2 < length:
        #     grad_top1_b2 = m.weight.grad.detach().view(-1)[w_idx_top1+2].float()

        # surroud_grad_top1 = [grad_top1_f2, grad_top1_f1, grad_top1, grad_top1_b1, grad_top1_b2]

        # print(f'{n}:{surroud_grad_top1}')
        # --------------------------
        ...
        while self.loss_max <= self.loss.item():
            ...
            for name, module in model.named_modules():
                if isinstance(module, quan_Conv2d) or isinstance(
                    ...
                    attack_weight,grad_max,b_grad_topk = self.flip_bit(name, module)
...
def visualize_grad(model,num):
    ...
    for name, module in model.named_modules():
        if isinstance(module, quan_Conv2d) or isinstance(module, quan_Linear):
            ...
            grads_mean = torch.max(grads.abs())
    ...
    df = pd.DataFrame({'Layer Name': layer_names, 'Average Gradient': aver_grads})
    df.to_csv('/home/bobzhou/BFA/'+output_path_grad+'/grads_mean.csv', index=False)
