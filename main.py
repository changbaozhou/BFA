...
parser.add_argument(
    '--dataset',
    type=str,
    choices=['cifar10', 'cifar100', 'imagenet', 'svhn', 'stl10', 'mnist'],default='cifar10',
...
def main():
    ...
    if args.optimizer == "SGD":
        ...
        print("using SAM as optimizer")
        optimizer = torch.optim.SGD
        optimizer = SAM(all_param, 
                        optimizer,
                        lr=state['learning_rate'], 
                        momentum=state['momentum'], 
                        weight_decay=state['decay'], 
                        rho=args.rho, adaptive=False)
    ...
    '''reset_weight在训练网络是设为false,BFA时设为true,意思是将权重设为整数量化值'''
    ...
    # if args.bin:
    #     attacker = BFA_bin_back.BFA(criterion, net, args.k_top)
    # else:
    #     
    attacker = BFA(criterion,args.k_top)
    ...
    if args.enable_bfa:
        perform_attack(attacker, net, net_clean, train_loader, test_loader,
                       target_acc, log, writer)
        # perform_attack_N(attacker, net, net_clean, train_loader, test_loader,
...
def perform_attack_N(attacker, model, model_clean, train_loader, test_loader,
                   N_iter, log, writer):
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    attacked_val_acc_top1 = val_acc_top1
    i_iter = 0
    accuracy_track = []
    attacked_layers = []
    grads = []
    b_grad_topks = []
    while attacked_val_acc_top1 > target_acc:
    # for i_iter in range(N_iter):
        print_log('**********************************', log)
        # attacked_layer = attacker.progressive_bit_search(model, data, target)
        if args.bin:
            attacked_layer = attacker.progressive_bit_search(model, data, target)
            attacked_layers.append(attacked_layer)
        else:
            attacked_layer,grad_max,b_grad_topk = attacker.progressive_bit_search(model, data, target)
            attacked_layers.append(attacked_layer)
            grads.append(grad_max)
            b_grad_topks.append(b_grad_topk)
        ...
        accuracy_track.append(val_acc_top1)
