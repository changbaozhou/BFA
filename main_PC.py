...
def main():
    ...
    print_log("=> network :\n {}".format(net), log)
    # visulizing model architecture
    # writer.add_graph(net, input_to_model=(torch.rand(32,3,32,32),))
    # if args.dataset == 'imagenet':
    #     writer.add_graph(net, input_to_model=(torch.rand(32,3,224,224),))
    # else:
    #     writer.add_graph(net, input_to_model=(torch.rand(32,3,32,32),))
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    grads = []
    b_grad_topks = []
    while attacked_val_acc_top1 > 11:
    # for i_iter in range(N_iter):
        print_log('**********************************', log)
