...
if args.optimizer == "SGD":
        args.checkpoint_name = args.arch+"_"+args.dataset+"_"+args.optimizer
elif args.optimizer == "SAM":
        args.checkpoint_name = args.arch+"_"+args.dataset+"_"+args.optimizer+"_rho"+str(args.rho)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint(checkpoint_state, is_best, './checkpoint/',
                        args.checkpoint_name, log)

        # measure elapsed time
        epoch_time.update(time.time() - start_time)
        start_time = time.time()
        recorder.plot_curve(os.path.join(args.save_path, '%s.png'%args.checkpoint_name))
...
def save_checkpoint(state, is_best, save_path, filename, log):
    ...
    if is_best:  # copy the checkpoint to the best model if it is the best_accuracy
        torch.save(state, "%s.pth" %filename)
